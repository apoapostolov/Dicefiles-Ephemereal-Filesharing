...
export default new (class Messages extends EventEmitter {
  ...
  toGifEmbed(url) {
    try {
      ...
      if (ext) {
        return {
          type: ext.toLowerCase() === "gif" ? "image" : "video",
          src: u.href,
        };
      }
      ...
      if (host.endsWith("giphy.com")) {
        const id =
          (path.match(/\/media\/([a-zA-Z0-9]+)\//) || [])[1] ||
          (path.match(/-([a-zA-Z0-9]+)\/?$/) || [])[1];
        if (id) {
          return {
            type: "image",
  ...
  addWelcome() {
    ...
    const renameWrap = tpl.querySelector(".welcome_namewrap");
    const renameInput = tpl.querySelector(".welcome_nameinput");
    ...
    const syncRenameUi = () => {
      ...
      renameWrap.classList[show ? "remove" : "add"]("hidden");
      if (!show || !renameInput) {
        return;
      }
      const rn = registry.config.get("roomname");
      renameInput.value = typeof rn === "string" ? rn : "";
    };
    ...
    if (renameWrap && renameInput && renameBtn) {
      ...
      registry.socket.on("role", syncRenameUi);
      registry.socket.on("owner", syncRenameUi);
