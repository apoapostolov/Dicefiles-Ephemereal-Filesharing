...
const TIPMETA = Object.freeze(["duration", "codec", "bitrate", "pages"]);
...
class FileTooltip extends Tooltip {
  ...
  addPreview(file) {
    if (file.meta && file.meta.requestImageDataUrl) {
      const img = new Image();
      img.src = file.meta.requestImageDataUrl;
      img.setAttribute("alt", `Request preview for ${file.name}`);
  ...
  isBookLike() {
    ...
    const t = ((file.meta && file.meta.type) || "").toLowerCase();
