...
// Track preview/asset generation queue depth for /healthz.
const _assetsPoolLimit = CONFIG.get("maxAssetsProcesses") || 2;
let _assetsPending = 0;
let _assetsActive = 0;

function _reportPreviewQueue() {
  if (OBS && typeof OBS.setPreviewQueue === "function") {
    OBS.setPreviewQueue({
      pending: _assetsPending,
      active: _assetsActive,
    });
  }
}

function _wrapQueueAware(limit, ctx, fn) {
  const wrapped = wrap(limit, ctx, async function (...args) {
    _assetsPending = Math.max(0, _assetsPending - 1);
    _assetsActive += 1;
    _reportPreviewQueue();
    try {
      ...
      _assetsActive = Math.max(0, _assetsActive - 1);
      _reportPreviewQueue();
    }
  });
...
const PDFTOPPM = CONFIG.get("pdftoppm") || "pdftoppm";
...
const FALLBACK_COMIC_EXTS = Object.freeze(
  new Map([
    [".cbz", "CBZ"],
    [".cbr", "CBR"],
    [".cbt", "CBT"],
...
const FALLBACK_DOCUMENT_EXTS = Object.freeze(
  new Map([
    [".pdf", ["application/pdf", "PDF"]],
    [".epub", ["application/epub+zip", "EPUB"]],
    [".mobi", ["application/octet-stream", "MOBI"]],
    [".azw", ["application/octet-stream", "AZW"]],
    [".azw3", ["application/octet-stream", "AZW3"]],
  ]),
);

/**
 * Keep common previewable files correctly classified when ExifTool is absent
 * or rejects an input. `file --mime-type` plus the upload extension is enough
 * for images, PDFs, media, comics, and ordinary archives.
 */
function classifyDetectedType(name, detectedMime) {
  const ext = path.extname(name || "").toLowerCase();
  const mime = (detectedMime || "").trim().toLowerCase();
  const comicType = FALLBACK_COMIC_EXTS.get(ext);
  if (comicType) {
    return {
      type: "document",
  ...
  const documentType = FALLBACK_DOCUMENT_EXTS.get(ext);
  if (documentType) {
    return {
      type: "document",
      mime: documentType[0],
      metaType: documentType[1],
    };
  }

  const mediaMatch = mime.match(/^(image|video|audio)\//);
  if (mediaMatch) {
    return {
      type: mediaMatch[1],
      mime,
      metaType: ext.slice(1).toUpperCase() || mediaMatch[1].toUpperCase(),
    };
  }

  if (mime === "application/pdf") {
    return { type: "document", mime, metaType: "PDF" };
  }
  if (mime === "application/epub+zip") {
    return { type: "document", mime, metaType: "EPUB" };
  }
  if (
    FALLBACK_TYPEMAP.get(ext) === "archive" ||
    /(?:zip|rar|7z|tar|gzip|bzip2|xz)/.test(mime)
  ) {
    return {
      type: "archive",
      mime: mime || "application/octet-stream",
      metaType: ext.slice(1).toUpperCase() || "ARCHIVE",
    };
  }
  if (FALLBACK_TYPEMAP.get(ext) === "document" || mime.startsWith("text/")) {
    return {
      type: "document",
      mime: mime || "text/plain",
      metaType: ext.slice(1).toUpperCase() || "TEXT",
    };
  }
  return {
    type: "file",
    mime: mime || "application/octet-stream",
    metaType: ext.slice(1).toUpperCase() || "BINARY",
  };
}
...
async function generateAssetsPdf(storage) {
  const assets = [];
  const known = new Set();
  const fromGraphicsMagick = async () => {
    if (!gm) {
      throw new Error("GraphicsMagick adapter is unavailable");
    }
    const fromPath = () =>
  ...
  const fromPoppler = async () => {
    ...
    const [buffer] = await runcmd(args, null);
    if (!buffer || !buffer.length) {
      throw new Error("Poppler returned an empty PDF preview");
    }
    return buffer;
  };
  ...
  let prev;
  try {
    prev = await fromGraphicsMagick();
  }
  catch (gmError) {
    console.warn(
      `GraphicsMagick PDF preview failed for ${storage.full}; trying Poppler`,
      gmError,
    );
    prev = await fromPoppler();
  }
...
async function generateAssetsImage(storage) {
  let { meta: { width, height } = {} } = storage;
  if (!width || !height) {
    const dimensions = await sharp(storage.full, {
      limitInputPixels: PIXEL_LIMIT,
    }).metadata();
    ({ width, height } = dimensions);
    if (width && height) {
      storage.meta.width = width;
      storage.meta.height = height;
    }
  }
...
async function generateAssets(storage) {
  try {
    const { mime } = storage;
    if (storage.meta.type === "PDF" || mime === "application/pdf") {
...
async function getMetaData(storage, name) {
  ...
  try {
    ...
    const fallback = classifyDetectedType(name, mime);
    rv.type = fallback.type;
    rv.mime = fallback.mime;
    rv.meta.type = fallback.metaType;
    if (rv.type === "image") {
      try {
        const dimensions = await sharp(file, {
          limitInputPixels: PIXEL_LIMIT,
        }).metadata();
        if (dimensions.width && dimensions.height) {
          rv.meta.width = dimensions.width;
          rv.meta.height = dimensions.height;
        }
      }
      catch (dimensionsError) {
        console.warn(
          `Could not read fallback image dimensions for ${file}`,
          dimensionsError,
        );
      }
    }
    console.warn(
      `ExifTool metadata unavailable for ${name}; detected ${rv.type}/${rv.mime}`,
      ex.message || ex,
    );
    return rv;
  }
...
module.exports = {
  ...
  generateAssets: _wrapQueueAware(
  ...
  extractComicPage: _wrapQueueAware(
    CONFIG.get("maxAssetsProcesses"),
    null,
    extractComicPage,
  ),
  ensureComicAssets: _wrapQueueAware(
    CONFIG.get("maxAssetsProcesses"),
    null,
    ensureComicAssets,
  ),
