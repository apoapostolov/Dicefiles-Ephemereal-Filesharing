...
function event(type, payload = {}) {
  appendLine(lifecycleLog, Object.assign({
...
function trackUploadCreated(upload) {
  ...
  event("upload_created", Object.assign(roomAndUser(upload), {
    key: upload.key,
    name: upload.name,
    size: upload.size,
...
function trackUploadDeleted(upload, reason = "deleted") {
  ...
  event("upload_deleted", Object.assign(roomAndUser(upload), {
    reason,
    key: upload.key,
    name: upload.name,
    size: upload.size,
...
function trackRequestCreated(request) {
  ...
  event("request_created", Object.assign(roomAndUser(request), {
    key: request.key,
    text: request.name,
...
function trackRequestFulfilled(request) {
  ...
  event("request_fulfilled", Object.assign(roomAndUser(request), {
    key: request.key,
    text: request.name,
    uploaded: request.uploaded,
  }));
