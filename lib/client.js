...
const { EMITTER: REQUESTS, REQUESTS: REQMAP } = require("./request");
...
const DYING = new CoalescedUpdate(20000, clients =>
...
BROKER.on("reqresumptions", stoken => {
  ...
  BROKER.emit(
    `resumptions:${stoken}`,
    msgpack.encode(c.resumptions).toString("binary"),
  );
});

const { Channels } = require("./client/channels");
...
class Client {
  constructor(room, socket, rtoken) {
    ...
    this.session = null;
    this.memberJoinedAt = 0;
    const { nick, cv, s, n, v } = socket.handshake.query;
    ...
    */
    if (session) {
      ...
      const signedOk = verifier.verify(secret, session, s, n);
      const plainOk = v && v === verifier.generate(secret, session);
      if (signedOk || plainOk) {
    ...
    socket.on("requeststatus", this.onrequestStatus.bind(this));
    socket.on("probelinks", this.onprobelinks.bind(this));
    socket.on("getlinks", async () => {
      try {
        const { EMITTER: LINKS_EMITTER } = require("./links");

        const links = await LINKS_EMITTER.for(this.room);
        this.emitOnce("links", {
          replace: true,
          links: links.map(l => l.toClientJSON()),
        });
      }
      catch (ex) {
        console.error("getlinks failed", ex);
      }
    });
    ...
    this.room.on("links", this.onlinks);
  ...
  async hellbanned() {
    return (
      this.role !== "mod" &&
      !!(await bans.findBan("hellban", this.ip, this.user && this.user.account))
    );
  }
  ...
  async onsession(session) {
    ...
    if (this.user) {
      BROKER.removeListener(
        `reqpubkey:${this.user.account}`,
        this.onbreqpubkey,
      );
      BROKER.removeListener(
        `${this.roomid}:privmsg:${this.user.account}`,
        this.onbprivmsg,
      );
    }
    this.user = this.session ? await User.load(this.session) : null;
    this.memberJoinedAt = await this.room.recordMemberJoin(
  ...
  async ontrash(files) {
    try {
      let num = 0;
      if (this.privileged) {
        num = await this.room.trash(files);
      }
      else {
        num = await this.room.trashOwned(
          files,
          this.ip,
          this.user && this.user.account,
        );
        if (!num) {
          throw new Error("You can remove only your own files");
        }
      }
  ...
  async onremovemessage(id, options) {
    try {
      ...
      if (!options.user && !options.ip) {
        ...
        await redis.removemessages(id, options.user, options.ip, options.room);
      }
  ...
  async setConfig(name, arg) {
    ...
    case "name":
      ...
      this.room.config.set("roomnamecustom", true);
      return;
    ...
    case "getPasswordAccessAdminState":
      return this.room.getPasswordAccessAdminState(true);

    case "configurePasswordAccess":
      return this.room.configurePasswordAccess(arg);

    case "rotatePasswordAccess":
      return this.room.rotatePasswordAccess(arg);
    ...
    case "allowRequests":
      this.room.config.set("allowRequests", !!arg);
      return;

    case "linkCollection":
      this.room.config.set("linkCollection", !!arg);
      return;

    case "linkedRooms": {
      const next = await this.room.setLinkedRooms(arg);
      // Refresh file list so linked rows appear/disappear without rejoin.
      try {
        const files = await this.room.getFilesFor(this);
        this.emitOnce("files", { replace: true, files });
      }
      catch (ex) {
        console.error("linkedRooms refresh", ex);
      }
      return next;
    }

    case "federatedRooms": {
      const next = this.room.setFederatedRooms(arg);
      try {
        const files = await this.room.getFilesFor(this);
        this.emitOnce("files", { replace: true, files });
      }
      catch (ex) {
        console.error("federatedRooms refresh", ex);
      }
      return next;
    }

    case "deepLinks":
      this.room.setDeepLinks(arg);
      return;

    case "allowCrossLinking":
      this.room.setAllowCrossLinking(arg);
      // Other rooms may be listing us — no local list change required here.
      return !!this.room.config.get("allowCrossLinking");

    case "allowPrivateCrossLinking":
      this.room.setAllowPrivateCrossLinking(arg);
      // Bilateral private-source consent is enforced when links are probed
      // and when source files are collected.
      return !!this.room.config.get("allowPrivateCrossLinking");

    case "allowFederation":
      return this.room.setAllowFederation(arg);

    case "allowPrivateFederation":
      return this.room.setAllowPrivateFederation(arg);

    case "createGuestInvite": {
      const o = arg && typeof arg === "object" ? arg : {};
      return this.room.createGuestInviteLink({
        singleUse: o.singleUse === true,
        maxUses: o.maxUses,
        maxAgeHours: o.maxAgeHours,
        label: o.label,
        createdBy: (this.user && this.user.account) || "",
      });
    }

    case "listGuestInvites":
      return this.room.listGuestInviteLinks();

    case "getGuestInviteAdminState":
      return this.room.getGuestInviteAdminState();

    case "revokeGuestInvite":
      return this.room.revokeGuestInviteLink(
        typeof arg === "string" ? arg : arg && arg.token,
        (this.user && this.user.account) || "",
      );

    case "listPluginCatalog": {
      const { defaultRoomPluginRuntime } = require("./plugins/room-runtime");

      const roomPlugins = await Promise.all(
        this.room.listRoomPluginsDetailed().map(async plugin => {
          const output = Object.assign({}, plugin);
          const state = await this.room.
            inspectRoomPluginSyncMemory(plugin.id, { limit: 1 }).
            catch(() => null);
          output.lastRun = state && state.lastRun;
          output.syncMemory = state ?
            {
    ...
    case "listRoomPlugins":
      return this.room.listRoomPluginsDetailed();

    case "inviteRoomPlugin": {
      const o = arg && typeof arg === "object" ? arg : {};
      return this.room.inviteRoomPlugin({
        id: o.id,
    ...
    case "revokeRoomPlugin":
      return this.room.revokeRoomPlugin(
        typeof arg === "string" ? arg : arg && arg.id,
      );

    case "setRoomPlugins":
      return this.room.setRoomPlugins(arg);

    case "runRoomPlugin": {
      const id =
          typeof arg === "string" ? arg : arg && (arg.id || arg.pluginId);
      return this.room.runRoomPluginNow(id);
    }

    case "inspectRoomPluginSyncMemory": {
      const o = arg && typeof arg === "object" ? arg : {};
      return this.room.inspectRoomPluginSyncMemory(o.id || o.pluginId, {
        limit: o.limit,
      });
    }

    case "clearRoomPluginSyncMemory": {
      const o = arg && typeof arg === "object" ? arg : {};
      return this.room.clearRoomPluginSyncMemory(o.id || o.pluginId, {
  ...
  async onsetconfig(name, arg) {
    // makeCall listens for `setconfig-${name}` and resolves with the payload.
    // Guest invite mint/list (and other setConfig returns) must be forwarded.
    const { runSetConfigRpc } = require("./client/setconfig-rpc");

    await runSetConfigRpc(
      (n, a) => this.setConfig(n, a),
      this.socket,
      name,
      arg,
    );
  ...
  /**
   * Linking-tab probe: return stored links with live source status.
   * Privileged (owner/mod) only — same gate as setconfig linkedRooms.
   */
  async onprobelinks() {
    try {
      this.ensurePrivilege();
      const rows = await this.room.probeLinkedRooms();
      this.socket.emit("probelinks", { links: rows });
    }
    catch (ex) {
      this.socket.emit("probelinks", {
        err: ex.message || ex.toString(),
      });
    }
  }

  async onprobefederationlinks() {
    try {
      this.ensurePrivilege();
      const rows = await this.room.probeFederatedRooms();
      this.socket.emit("probefederationlinks", { links: rows });
    }
    catch (ex) {
      this.socket.emit("probefederationlinks", {
        err: ex.message || ex.toString(),
      });
    }
  }
  ...
  async init() {
    ...
    const rawLC = this.room.config.get("linkCollection");
    const effectiveLC =
      rawLC !== undefined && rawLC !== null ?
        !!rawLC :
        CONFIG.get("linkCollection") !== false;
    const links = effectiveLC ? await LINKS_EMITTER.for(this.room) : [];
    this.emitOnce("links", {
      replace: true,
      links: links.map(l => l.toClientJSON()),
    });
  ...
  async broadcast(msg) {
    ...
    for (;;) {
      ...
      await Promise.all([
        redis.set(
          `massoc:a:${messageInfo.a}:${this.roomid}:${id}`,
          1,
          "EX",
          2 * 60 * 60,
        ),
        redis.set(
          `massoc:i:${messageInfo.i}:${this.roomid}:${id}`,
          1,
          "EX",
          2 * 60 * 60,
        ),
  ...
  async onrequest(payload, callback) {
    try {
      // Gate: check whether request creation is enabled for this room
      const rawAR = this.room.config.get("allowRequests");
      const effectiveAR =
        rawAR !== undefined && rawAR !== null ?
          !!rawAR :
          CONFIG.get("allowRequests") !== false;
      if (!effectiveAR) {
        throw new Error("Request creation is disabled in this room");
      }
      ...
      if (this.user && this.user.account) {
        await this.user.addCreatedRequest();
      }
      await this.broadcast({
        role: "system",
        user: "System",
        msg: await toMessage(
          `REQUEST by ${this.nick}: ${text}${requestUrl ? ` (${requestUrl})` : ""}`,
        ),
      });
      ...
      if (typeof callback === "function") {
        callback({ err: ex.message || ex.toString() });
      }
      this.unicast({
        user: "Error",
        role: "system",
  ...
  async onrequestStatus(payload, callback) {
    try {
      const key =
      ...
      if (!["fulfilled", "open", "removed"].includes(status)) {
      ...
      if (req.roomid !== this.roomid) {
        throw new Error("Request not found in this room");
      }
      // "removed" requires mod
      if (status === "removed" && !this.privileged) {
        throw new Error("Only moderators can remove requests");
      }
      if (status === "removed") {
        REQMAP.delete(key);
      }
      else {
        await REQUESTS.setStatus(key, status, this.nick);
        if (
          status === "fulfilled" &&
          ...
          await this.user.addFulfilledRequest();
        }
      }
      if (typeof callback === "function") {
        callback({ ok: true });
  ...
  async onmessage(msg) {
    ...
    try {
      ...
      const parsedMsg = await toMessage(msg);
      const isHellbanned = await this.hellbanned();
      await this.broadcast({
        msg: parsedMsg,
        hellbanned: isHellbanned,
      });

      // Touch room activity so the pruner won't delete active rooms
      if (!isHellbanned) {
        this.room.touchActivity();
      }

      if (!isHellbanned) {
        const rawLC2 = this.room.config.get("linkCollection");
        const collectLinks =
          rawLC2 !== undefined && rawLC2 !== null ?
            !!rawLC2 :
            CONFIG.get("linkCollection") !== false;
        if (collectLinks) {
          const { Link } = require("./links");

          for (const part of parsedMsg) {
            if (part.t === "u") {
              // ignore GIF provider links (Giphy + legacy Tenor CDN) for archive
              try {
                const uobj = new URL(part.v);
                const host = uobj.hostname.toLowerCase();
                if (host.endsWith("giphy.com") || host.endsWith("tenor.com")) {
                  continue;
                }
              }
              catch (_) {
                // malformed URL? skip link creation as normal
              }
              ...
              try {
                await Link.create({
                  roomid: this.roomid,
                  url: part.v,
                  name: part.v, // We can resolve this later or just use URL
                  sharer: this.nick,
                });
              }
              catch (ex) {
                console.error("Failed to create link", ex);
              }
            }
          }
        }
  ...
  onconfig(key, value) {
    ...
    if (key === "passwordAccess" || key === "passwordAccessEpoch") {
      this.room.
        passwordAccessAllowed(
          this.user,
          this.token,
          this.socket.handshake.cookies || {},
        ).
        then(allowed => {
          if (!allowed) {
            this.emit("outdated");
          ...
          console.error("password access recheck failed", error),
        );
    }
    if (key === "disabled") {
      this.disabled = value;
  ...
  onlinks(action, links) {
    switch (action) {
    case "add":
      if (!links.length) {
        return;
      }
      this.emitOnce("links", { links: links.map(l => l.toClientJSON()) });
      return;
    ...
    case "deleted":
      ...
      this.emitOnce("links-deleted", links);
      return;

    case "updated":
      this.emitOnce(
        "links-updated",
        links.map(l => l.toClientJSON()),
      );
      return;
    }
  }
  ...
  onreallyclose() {
    ...
    this.room.removeListener("links", this.onlinks);
    this.room.unref();
