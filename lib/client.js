...
class Client {
  ...
  async init() {
    ...
    const effectiveLC = rawLC !== undefined && rawLC !== null ? !!rawLC : CONFIG.get("linkCollection") !== false;
  ...
  async onrequest(payload, callback) {
    try {
      ...
      const effectiveAR = rawAR !== undefined && rawAR !== null ? !!rawAR : CONFIG.get("allowRequests") !== false;
      if (!effectiveAR) {
  ...
  async onmessage(msg) {
    ...
    try {
      ...
      if (!isHellbanned) {
        ...
        const collectLinks = rawLC2 !== undefined && rawLC2 !== null ? !!rawLC2 : CONFIG.get("linkCollection") !== false;
        if (collectLinks) {
