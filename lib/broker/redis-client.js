...
function legacyScoreReply(entries) {
  ...
  return entries.flatMap(entry => [entry.value, String(entry.score)]);
}
...
async function invokeCommand(client, name, args) {
  ...
  if (cmd === "zrevrange" || cmd === "zrange") {
    const withScores =
      typeof callArgs[callArgs.length - 1] === "string" &&
      callArgs[callArgs.length - 1].toUpperCase() === "WITHSCORES";
    if (withScores) {
      callArgs = callArgs.slice(0, -1);
    }
    const [key, start, stop] = callArgs;
    const reverse = cmd === "zrevrange";

    if (withScores) {
      const reverseScores = client.zRevRangeWithScores;
      if (reverse && typeof reverseScores === "function") {
        return legacyScoreReply(
          await reverseScores.call(client, key, start, stop),
        );
      }
      ...
      const reverseRange = client.zRevRange;
      if (reverse && typeof reverseRange === "function") {
        return reverseRange.call(client, key, start, stop);
      }
    ...
    // Compatible fallback for clients exposing only raw commands. Do not pass
    // a JSON options object: Redis expects the literal WITHSCORES token.
    const rawArgs = [
      cmd.toUpperCase(),
      Buffer.isBuffer(key) ? key : String(key),
      String(start),
      String(stop),
    ];
    if (withScores) {
      rawArgs.push("WITHSCORES");
    }
    return client.sendCommand(rawArgs);
  }
  if (cmd === "set") {
    ...
    if (callArgs.length >= 3 && typeof callArgs[1] !== "object") {
      callArgs = callArgs.map((a, i) =>
        i === 0 ?
          String(a) :
          typeof a === "object" && a !== null && !Buffer.isBuffer(a) ?
            a :
            String(a),
      );
    }
