...
async function prepareRoomPasswords() {
  try {
    await Room.preparePasswordAccess();
  } catch (ex) {
    console.error("Room password preparation failed", ex);
  }
}
...
setTimeout(() => {
  prepareRoomPasswords();
  setInterval(prepareRoomPasswords, HOUR_MS);
}, 30 * 1000);
