...
async function generateAssetsEpubMobi(storage) {
  ...
  if (fileType === "EPUB") {
    ...
    // Convert MOBI → EPUB so the browser reader can open it (epubjs cannot parse MOBI)
    await convertMobiToEpub(storage);

    if (EXIFTOOL) {
      // MOBI/AZW files embed the cover as a raw binary blob; exiftool can extract it
      let exiftool;
      if (JAIL) {
        ...
        exiftool = [EXIFTOOL, "-b", "-CoverImage", storage.full];
      }
      try {
        ...
        console.error("Failed to extract MOBI cover via exiftool", ex);
        OBS.trackPreviewFailure(storage, "mobi-cover-exiftool", ex);
      }
    }
  }
...
/**
 * Convert a MOBI/AZW file to EPUB using calibre's ebook-convert and store the
 * result as a .converted.epub asset.  Safe to call on already-converted files.
 */
async function convertMobiToEpub(storage) {
  if (storage.assets.has(".converted.epub")) {
    return; // already done
  }
  const convertedPath = storage.full + ".converted.epub";
  try {
    // ebook-convert writes the EPUB to convertedPath; outputs to stderr (ignored)
    await runcmd(["ebook-convert", storage.full, convertedPath], "utf-8");
    // File is now at storage.full + ".converted.epub" — register without re-copying
    await storage.addAssets([
      {
        ext: ".converted.epub",
        type: "document",
        mime: "application/epub+zip",
        file: convertedPath, // already written by ebook-convert
      },
    ]);
  } catch (ex) {
    console.error("Failed to convert MOBI to EPUB via ebook-convert", ex);
    OBS.trackPreviewFailure(storage, "mobi-epub-convert", ex);
    try {
...
async function generateAssets(storage) {
  try {
    ...
    if (storage.meta.type === "EPUB" || storage.meta.type === "MOBI") {
