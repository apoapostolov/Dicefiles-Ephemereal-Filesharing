...
async function rarListImages(archivePath) {
  ...
  const all = out.toString("utf8").split(/\r?\n/).map(l => l.trim()).filter(Boolean);
  const imgs = all.filter(l => COMIC_IMAGE_EXTS.has(path.extname(l).toLowerCase()));
  imgs.sort((a, b) =>
...
async function rarExtractFile(archivePath, fileName) {
...
function parseComicInfoXml(xmlStr) {
  const rv = {};
  for (const field of ["Title", "Series", "Number", "Year", "Month",
    "Writer", "Penciller", "Publisher", "Summary", "PageCount"]) {
    const m = xmlStr.match(new RegExp(`<${field}[^>]*>([^<]*)<\\/${field}>`, "i"));
    if (m) {
...
async function rarReadComicInfo(archivePath) {
  ...
  const all = out.toString("utf8").split(/\r?\n/).map(l => l.trim()).filter(Boolean);
  const entry = all.find(l => path.basename(l).toLowerCase() === "comicinfo.xml");
...
async function generateAssetsComic(storage) {
  ...
  if (container === "zip") {
    ...
    const files = [];
    zip.forEach((relativePath, entry) => {
      if (!entry.dir && COMIC_IMAGE_EXTS.has(path.extname(relativePath).toLowerCase())) {
    ...
    try {
      imageFiles = await rarListImages(storage.full);
    } catch (ex) {
      console.error("generateAssetsComic: failed to list RAR pages:", ex.message);
      OBS.trackPreviewFailure(storage, "cbz-rar-list", ex);
      return;
    }
    getPageBuffer = (name) => rarExtractFile(storage.full, name);
    comicInfoXml = await rarReadComicInfo(storage.full);
  } else {
    console.warn(`generateAssetsComic: unknown container format for ${storage.full}`);
    return;
  }

  const pageCount = imageFiles.length;
  if (pageCount === 0) {
    console.warn(`generateAssetsComic: no image files found in ${storage.full}`);
    await storage.addAssets([]);
    return;
  }
  ...
  if (comicInfoXml) {
    ...
    if (info.title) storage.meta.comic_title = info.title;
    if (info.series) storage.meta.comic_series = info.series;
    if (info.number) storage.meta.comic_number = info.number;
    if (info.year) storage.meta.comic_year = info.year;
    if (info.publisher) storage.meta.comic_publisher = info.publisher;
    if (info.writer) storage.meta.comic_writer = info.writer;
  ...
  if (storage.meta.comic_cover_idx !== undefined) {
    ...
    const coverHeuristic = imageFiles.findIndex(
      f => /\bcover\b/i.test(path.basename(f, path.extname(f))),
    );
    if (coverHeuristic >= 0) {
  ...
  try {
    ...
    console.error("generateAssetsComic: cover sharp processing failed:", ex.message);
 ...
 */
async function ensureComicAssets(storage) {
...
async function extractComicPage(storage, n) {
  ...
  if (storage.meta && storage.meta.comic_index) {
    ...
    pageFiles = await rarListImages(storage.full);
  } else {
    throw new Error(`extractComicPage: unsupported container for ${storage.full}`);
  }
  ...
  if (container === "zip") {
    ...
    // RAR
    raw = await rarExtractFile(storage.full, pageFiles[pageIdx]);
