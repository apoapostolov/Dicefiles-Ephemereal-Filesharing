...
function federationRuleFile(upload) {
  return {
    name: upload.name,
    type: upload.type,
    tags: upload.tags || {},
...
function requestedFederationRules(raw) {
  if (raw == null || raw === "") {
    return { valid: true, rules: null, errors: {} };
  }
  ...
  if (input.length > 5000) {
    return {
      valid: false,
      rules: null,
      errors: { rules: "Federation rules are too large." },
    };
  }
  let parsed;
  try {
    ...
    return {
      valid: false,
      rules: null,
      errors: { rules: "Federation rules must be valid JSON." },
    };
  }
  return validateLinkRules(parsed);
}
