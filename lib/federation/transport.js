...
async function responseFederationError(response, peerId, maxBytes) {
  let remoteCode = "";
  try {
    const value = await readBoundedJson(response, maxBytes);
    remoteCode = String(value && value.error && value.error.code || "");
  }
  catch (_) {
    // Preserve status-based behavior when a peer returns HTML or malformed JSON.
  }
  ...
  if (signatureCodes.has(remoteCode)) {
    return new FederationError(
      `Peer ${peerId} rejected the configured federation key`,
      "FEDERATION_KEY_INVALID",
      response.status,
    );
  }
  if (remoteCode === "FEDERATION_PROTOCOL_MISMATCH") {
    return new FederationError(
      `Peer ${peerId} uses an incompatible federation protocol`,
...
async function signedFetch(peerId, pathname, options = {}) {
  ...
  let timeout;
  try {
    const signRequest = options._signRequest || signFederationRequest;
    const fetchRequest = options._fetch || fetch;
    ...
    const signed = await signRequest(unsigned, config);
    const controller = new AbortController();
    timeout = setTimeout(() => controller.abort(), limits.timeoutMs);
    ...
    if (!response.ok) {
      throw await responseFederationError(
        response,
        peerId,
        limits.maxJsonBytes,
      );
    ...
    const wrapped = new FederationError(
...
async function getRemoteRoomFiles(
    peerId,
    roomId,
    { refresh = false, rules = null } = {},
) {
  const config = currentConfig();
  const cacheKey = `${peerId}\0${roomId}\0${rulesCacheKey(rules)}`;
  ...
  for (let page = 0; page < 10 && files.length < 2000; page++) {
    ...
    if (rules && typeof rules === "object") {
      query.set("rules", JSON.stringify(rules));
    }
    const path =
...
function invalidateRemoteRoom(peerId, roomId) {
  const prefix = `${peerId}\0${roomId}\0`;
  for (const key of roomCache.keys()) {
...
async function probePeerRoom(peerId, roomId) {
  try {
    ...
    if (
      ...
      throw new FederationError(
        "Peer returned an incompatible room response",
        "FEDERATION_PROTOCOL_MISMATCH",
      );
    }
    return {
...
async function sendRoomInvalidation(peerId, activity) {
  const value =
  ...
  if (
    ...
    throw new FederationError(
      "Invalid federation invalidation activity",
