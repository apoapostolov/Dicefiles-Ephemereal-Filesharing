...
const FAKE = passwords.create("fake");

const KEY = Symbol();

const ADOPT_OFILTER = Object.freeze(
  new Set(["email", "pubmail", "message", "looking"]),
);

const UPSTAT_FILES = "uploadstat:files";
const UPSTAT_BYTES = "uploadstat:bytes";
const UPSTAT_REQUESTS = "uploadstat:requests";
const UPSTAT_REQUESTS_CREATED = "uploadstat:requests_created";
const UPSTAT_EXPIRE = "uploadstat:expire";
const DOWNSTAT_BYTES = "downloadstat:bytes";
const DOWNSTAT_EXPIRE = "downloadstat:expire";

const ACTIVITY_MAX = 20;
const activityKey = account => `activity:${account}`;

const UPSTAT_CACHE = new LRUCache({
  max: 1000,
});

const STATS_CACHE = new LRUCache({
  max: 100,
});
...
BROKER.on(UPSTAT_EXPIRE, acct => {
  UPSTAT_CACHE.delete(acct);
  STATS_CACHE.clear();
});

BROKER.on(DOWNSTAT_EXPIRE, acct => {
  UPSTAT_CACHE.delete(acct);
});
...
class User {
  ...
  async uploadStats() {
    ...
    try {
      ...
      const ub = await redis.zscore(UPSTAT_BYTES, this.account);
      if (ub) {
        rv.uploaded = parseInt(ub, 10) || 0;
        rv.uploadedRank =
          (await redis.zrevrank(UPSTAT_BYTES, this.account)) + 1;
      }
      const db = await redis.zscore(DOWNSTAT_BYTES, this.account);
      if (db) {
        rv.downloaded = parseInt(db, 10) || 0;
        rv.downloadedRank =
          (await redis.zrevrank(DOWNSTAT_BYTES, this.account)) + 1;
      }
      const rq = await redis.zscore(UPSTAT_REQUESTS, this.account);
      if (rq) {
        rv.fulfilledRequests = parseInt(rq, 10) || 0;
      }
      const rc = await redis.zscore(UPSTAT_REQUESTS_CREATED, this.account);
      if (rc) {
        rv.createdRequests = parseInt(rc, 10) || 0;
      }
      return rv;
    }
    finally {
  ...
  async changePassword(ip, current, password, token) {
    const verify = await redis.get(`user:pw:${this.account}`);
    const fake = await FAKE;
    const ok = await passwords.verify(verify || fake, current);
    if (!verify || !ok) {
      throw new Error("Invalid username or password");
    }

    if (this.twofactor && !token) {
      return {
        twofactor: true,
      };
    }

    if (this.twofactor && !verifyTwofactor(this.twofactor, token)) {
      throw new Error("Invalid two-factor authentication code!");
    }

    // Validate password strength via shared policy (lib/validate.js).
    validatePassword(password);

    password = await passwords.create(password);
    await redis.set(`user:pw:${this.account}`, password);
  ...
  async adopt(o) {
    ...
    if ((this.looking || "").length > 2000) {
      throw new Error("Looking-for message too long");
    }
    if (this.email) {
      // MD5 is used here only for Gravatar URL construction — non-security-sensitive;
      // it is not used for authentication or data integrity.
      const h = crypto.
        createHash("md5").
        update(this.email.toLowerCase()).
        digest("hex");
      try {
        // native fetch (Node 18+) replaces request-promise-native
        const res = await fetch(`https://gravatar.com/${h}.json`, {
          headers: { "User-Agent": "dicefiles/1.0 like irc" },
          signal: AbortSignal.timeout(8000),
        });
        if (!res.ok) {
          throw new Error(`Gravatar returned ${res.status}`);
        }
        const {
          entry: [info],
        } = await res.json();
        ...
        console.error("Failed to get gravatar", ex.message);
      }
    }
    await this.save();
  ...
  async addUpload(bytes) {
    ...
    UPSTAT_CACHE.delete(this.account);
    BROKER.emit(UPSTAT_EXPIRE, this.account);
  }

  async addDownload(bytes) {
    if (!isFinite(bytes) || bytes <= 0) {
      return;
    }
    await redis.zincrby(DOWNSTAT_BYTES, bytes | 0, this.account);
    UPSTAT_CACHE.delete(this.account);
    BROKER.emit(DOWNSTAT_EXPIRE, this.account);
  }

  async addFulfilledRequest() {
    await redis.zincrby(UPSTAT_REQUESTS, 1, this.account);
    UPSTAT_CACHE.delete(this.account);
    BROKER.emit(UPSTAT_EXPIRE, this.account);
  }

  async addCreatedRequest() {
    await redis.zincrby(UPSTAT_REQUESTS_CREATED, 1, this.account);
    UPSTAT_CACHE.delete(this.account);
    BROKER.emit(UPSTAT_EXPIRE, this.account);
  }

  async recordActivity(type, entry) {
    const key = activityKey(this.account);
    const ts = Date.now();
    await redis.zadd(key, String(ts), JSON.stringify({ type, ts, ...entry }));
    await redis.zremrangebyrank(key, 0, -(ACTIVITY_MAX + 1));
  }

  async getActivity() {
    const raw = await redis.zrevrange(
      activityKey(this.account),
      0,
      ACTIVITY_MAX - 1,
    );
    return raw.
  ...
  static async login(ip, nick, pass, token) {
    ...
    // Per-account progressive lockout (credential-stuffing protection).
    // Check before touching the password hash so the lockout cannot be bypassed
    // by racing concurrent requests.
    const afp = new FloodProtector(
      nick,
      "loginAccountFloods",
      CONFIG.get("loginAccountFloodTrigger"),
      CONFIG.get("loginAccountFloodDuration"),
    );
    if (await afp.check()) {
      console.warn(
        "[auth]",
    ...
    const fake = await FAKE;
    const ok = await passwords.verify(verify || fake, pass);
    if (!verify || !ok) {
      // Bump per-account counter on each failed attempt.
      await afp.bump();
      // Security event log for failed login attempts
      console.warn(
        "[auth]",
    ...
    if (passwords.needsUpgrade(verify)) {
      pass = await passwords.create(pass);
    ...
    if (user.twofactor && !verifyTwofactor(user.twofactor, token)) {
      // Security event log for failed 2FA
      console.warn(
        "[auth]",
        "Failed 2FA attempt from",
        ip,
        "for account",
        nick,
      );
    ...
    await fp.delete();
    // Reset per-account lockout counter on successful login.
    await afp.delete();
  ...
  static async create(ip, onick, pass) {
    ...
    const phrase = await passwords.create(pass);
