...
const ADOPT_OFILTER = Object.freeze(new Set(["email", "pubmail", "message"]));
...
class User {
  ...
  async changePassword(ip, current, password, token) {
    ...
    // P0.5 — 3.2: Validate password strength via shared policy (lib/validate.js).
  ...
  async adopt(o) {
    ...
    if (this.email) {
      ...
      const h = crypto
        ...
        .digest("hex");
      try {
        // P0.5 — 3.3: native fetch (Node 18+) replaces request-promise-native
  ...
  static async login(ip, nick, pass, token) {
    ...
    // P0.5 — 3.2: Per-account progressive lockout (credential-stuffing protection).
    ...
    if (!verify || !ok) {
      // P0.5 — 3.2: Bump per-account counter on each failed attempt.
      await afp.bump();
      // P0.5 — 3.2: Security event log for failed login attempts
      console.warn(
        "[auth]",
    ...
    if (user.twofactor && !verifyTwofactor(user.twofactor, token)) {
      // P0.5 — 3.2: Security event log for failed 2FA
      console.warn(
        "[auth]",
    ...
    await fp.delete();
    // P0.5 — 3.2: Reset per-account lockout counter on successful login.
    await afp.delete();
