...
const ADOPT_OFILTER = Object.freeze(
  new Set(["email", "pubmail", "message", "interests"]),
);

const ACTIVITY_KEY = (account) => `user:activity:${account}`;
...
class User {
  ...
  async adopt(o) {
    ...
    if ((this.interests || "").length > 200) {
      throw new Error("User interests too long");
    }
    if (this.email) {
  ...
  /**
   * Record an upload in the user's recent-activity sorted set.
   * Score = upload timestamp (ms); member = compact JSON stub.
   * The set is capped at ACTIVITY_MAX entries (oldest trimmed first).
   */
  static async recordRecentUpload(account, info) {
    const key = ACTIVITY_KEY(account);
    const score = info.uploaded || Date.now();
    ...
    // Keep only the newest ACTIVITY_MAX entries.
    await redis.zremrangebyrank(key, 0, -(ACTIVITY_MAX + 1));
  }

  /**
   * Return the most recent `limit` upload stubs for `account` (newest first).
   * Returns an empty array if no activity has been recorded.
   */
  static async getRecentUploads(account, limit = 20) {
    const key = ACTIVITY_KEY(account);
    ...
    for (const member of raw) {
      try {
        results.push(JSON.parse(member));
      } catch (_) {
