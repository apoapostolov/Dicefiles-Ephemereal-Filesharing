...
class User {
  ...
  async uploadStats() {
    ...
    try {
      ...
      if (db) {
        ...
        rv.downloadedRank = await redis.zrevrank(DOWNSTAT_BYTES, this.account) + 1;
      }
      return rv;
    }
    finally {
  ...
  async adopt(o) {
    ...
    if (this.email) {
      ...
      try {
        ...
        const {entry: [info]} = await res.json();
  ...
  static async login(ip, nick, pass, token) {
    ...
    if (!verify || !ok) {
      ...
      console.warn("[auth]", "Failed login attempt from", ip, "for account", nick);
      throw new Error("Invalid username or password");
    }
    ...
    if (user.twofactor && !verifyTwofactor(user.twofactor, token)) {
      ...
      console.warn("[auth]", "Failed 2FA attempt from", ip, "for account", nick);
      throw new Error("Invalid two-factor authentication code!");
    }
    await fp.delete();
