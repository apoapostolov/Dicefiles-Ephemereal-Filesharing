...
class User {
  ...
  async changePassword(ip, current, password, token) {
    ...
    // P0.5 — 3.2: Enforce same password strength policy as registration
    if (password.length < 10 || !/[a-zA-Z]/.test(password) || !/\d/.test(password)) {
      throw new Error("Password must be at least 10 characters and contain a letter and a digit");
    }
  ...
  static async create(ip, onick, pass) {
    ...
    // P0.5 — 3.2: Minimum 10 chars; must contain at least one letter and one digit
    if (pass.length < 10 || !/[a-zA-Z]/.test(pass) || !/\d/.test(pass)) {
      throw new Error("Password must be at least 10 characters and contain a letter and a digit");
    }
