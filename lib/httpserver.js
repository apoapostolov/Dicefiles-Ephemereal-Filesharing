...
// P0.5 — 3.6: BROKER needed for distributed rate limiting and room messages.
...
// P0.5 — 3.5: Periodic cleanup and diagnostics for AUTOMATION_RATE_STATE.
...
// P0.5 — 3.5 / 3.6: Distributed Redis-backed rate limiting for the automation
// API.  Falls back to the in-process Map on Redis errors so a transient Redis
...
function requireAutomation(scope = null) {
  // P0.5 — 3.6: Middleware is now async to await the distributed Redis-backed
  // rate limiter.  Unhandled rejections are caught and forwarded to next(err).
  return async function (req, res, next) {
    try {
...
app.use(
  // P0.5 — 3.3: Helmet upgraded from 3.x → 7.x.
  // xssFilter and ieNoOpen were removed in Helmet 5+; they set legacy headers
  // (X-XSS-Protection / X-Download-Options) that are no longer recommended.
...
app.post(
  ...
  rtokenize(async (req, res) => {
    const { u, p } = req.body || {};
    // P0.5 — 3.4: Validate username and password lengths before processing.
...
app.post(
  ...
  rtokenize(async (req, res) => {
    ...
    // P0.5 — 3.4: Validate username and password before processing.
    const validNick = requireString(u, "username", 32);
    const validPass = requireString(p, "password", 256);
...
app.post(
  ...
  rtokenize(async (req) => {
    ...
    // P0.5 — 3.4: Validate current and new password lengths.
