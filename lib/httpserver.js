...
function normalizeAutomationKeyEntry(entry, index) {
  ...
  const preset = (entry.preset || entry.role || "").toString().trim().toLowerCase();
...
function loadAutomationKeyConfig() {
  ...
  const records = items.
    map(normalizeAutomationKeyEntry).
    filter(Boolean);
...
function auditAutomationRequest(req, status, extra = {}) {
  appendJSONLine(AUTOMATION_AUDIT_LOG, Object.assign({
...
function requireAutomation(scope = null) {
  return function(req, res, next) {
    ...
    res.setHeader("X-Dicefiles-RateLimit-Reset", Math.ceil(rate.resetAt / 1000));
    return next();
  };
}

function logAutomationResponse(req, res, next) {
  if (!req.path.startsWith("/api/automation/") && !req.path.startsWith("/api/v1/")) {
    return next();
  }
...
function validateRequestPayload(payload) {
  ...
  if (requestImageDataUrl) {
    ...
    if (!/^data:image\/[a-z0-9.+-]+;base64,[a-z0-9+/=]+$/i.
      test(requestImageDataUrl)) {
...
app.get("/healthz", aroute(async (req, res) => {
...
app.post(v1("/auth/login"), requireAutomation("auth:login"), jroute(async req => {
...
app.post(
  ...
  jroute(async req => {
    ...
    const {text, requestUrl, requestImageDataUrl} = validateRequestPayload(req.body);
...
app.post(
  ...
  jroute(async req => {
    ...
    await UPLOAD.registerUploadKey(
      roomid,
      user.name,
      key,
      room.fileTTL
...
app.get(
  ...
  jroute(async req => {
    ...
    return {
      ...
      files: filtered.map(f => Object.assign({}, f, {
        isNew: isFinite(since) && since > 0 ? Number(f.uploaded) > since : false
      })),
    };
  }));
...
app.get("/:page", (req, res, next) => {
  ...
  if (PAGES.has(page)) {
    const pagename = page === "terms" ? "Terms of Service and Privacy Policy" : "The Rules";
