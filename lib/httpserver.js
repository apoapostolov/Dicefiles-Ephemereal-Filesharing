...
app.get("/", async function (req, res) {
  if (CONFIG.get("publicRooms")) {
    try {
      ...
      return render(res, "index", { v, publicRooms: true, rooms });
    } catch (ex) {
...
// First-party plugins (Mega folder import, …) — see core/plugins/
try {
  ...
  if (runtime.megaDownloader && runtime.megaDownloader.sdkAvailable) {
    ...
    console.log(
      "[plugins] megaDownloader: megajs not installed (yarn add megajs for live Mega sync)",
    );
  }
  defaultRegistry
    .startAll(runtime)
