...
function requireAutomation(req, res, next) {
  const keys = asArray(CONFIG.get("automationApiKeys"));
  if (!keys.length) {
...
app.post("/api/automation/auth/login", requireAutomation, jroute(async req => {
...
app.get(
  "/api/automation/uploads/:key/offset",
  requireAutomation,
  getAutomationUser,
  requireAutomationUser,
  jroute(async req => {
