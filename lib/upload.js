...
const { getMetaData, generateAssets, convertMobiToEpub } = require("./meta");
const {
  DistributedMap,
  DistributedSet,
  RemoteMap,
...
async function serve(req, res, next) {
  ...
  try {
    ...
    // Keep the raw asset key (e.g. ".converted.epub") before we overwrite `asset`
    // with the resolved asset object, so we can use it in the fallback below.
    const assetKey = asset;
    if (assetKey) {
      asset = storage.assets.get(assetKey);
      if (!asset) {
        // On-demand MOBI→EPUB conversion for files uploaded before the conversion
        // asset was introduced.  Runs once; subsequent requests will hit the cache.
        if (
          assetKey === ".converted.epub" &&
          storage.meta &&
          storage.meta.type === "MOBI"
        ) {
