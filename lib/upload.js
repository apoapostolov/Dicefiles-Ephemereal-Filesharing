...
const { sanitizeTagValue } = require("./upload-sanitize");
...
async function realupload(req, res) {
  try {
    ...
    try {
      ...
      pending = await redis.hgetall(pkey);
      if (!pending) {
      ...
      const expectedBytes =
        offset + (Number(cl) >= 0 ? Number(cl) : 0);
      ...
      if (!pending.volumeId) {
        pending.volumeId = reservation.volumeId;
        await redis.hset(pkey, "volumeId", reservation.volumeId);
      }
      pending.reservedBytes = reservation.bytes;
      await redis.hset(pkey, "reservedBytes", reservation.bytes);

      storage = new StorageLocation({
        name: key,
        volumeId: pending.volumeId || "legacy",
      });
          ...
          meta: Object.assign(
            {
              account,
              role,
              ...(Number(pending.memberJoinedAt) > 0 ?
                { roomMemberJoinedAt: Number(pending.memberJoinedAt) } :
                {}),
              ...(requesterNick ?
      ...
      // Revoke Key
      PENDING.delete(key);
      await releaseUpload(key, pending.volumeId);
...
async function registerUploadKey(roomid, user, key, ttl, memberJoinedAt = 0) {
  ...
  try {
    ...
    if (Number(memberJoinedAt) > 0) {
      await redis.hsetnx(ckey, "memberJoinedAt", Number(memberJoinedAt));
    }
...
function findActiveRoomUploadByHash(roomid, hash) {
...
function duplicateRoomContentError(upload) {
  const error = new Error("This file content already exists in the room");
  error.code = "DUPLICATE_ROOM_CONTENT";
  error.existingKey = upload && upload.key;
  return error;
}

async function createIngestedUpload({
  name,
  roomid,
  size,
  hash,
  ...
  const displayName = (user && String(user).trim()) || (isBot ? "Bot" : "");
  let tags;
  if (isBot) {
    tags = Object.assign(
      { usernick: displayName, bot: "true" },
      extraTags && typeof extraTags === "object" ? extraTags : {},
    );
  }
  else if (account) {
    tags = Object.assign(
  ...
  const upload = await Upload.create(
    {
      ...
      meta: Object.assign(
        {
          account: isBot ? "" : account || "",
          role: role || "white",
        },
        isBot ? { bot: true, botName: displayName } : {},
...
async function ingestFromBuffer({
  ...
  if (skipIfRoomHashExists) {
    const existing = findActiveRoomUploadByHash(roomid, hash);
    if (existing) {
      throw duplicateRoomContentError(existing);
    }
  }
  ...
  if (!newStorage) {
    ...
    const reservationKey = `ingest:${tempName}`;
    const reservation = await reserveUpload(reservationKey, buffer.length);
    tempStorage = new StorageLocation({
      name: tempName,
    ...
    try {
      ...
      await releaseUpload(reservationKey, reservation.volumeId);
      throw error;
    }
    try {
      const {
...
/**
 * Ingest a readable stream without buffering it in process memory.
 * The stream is written into a temporary storage slot, hashed while flowing,
 * and rejected once maxBytes is exceeded.
 */
async function ingestFromStream({
  ...
  if (!stream || typeof stream.pipe !== "function") {
    throw new Error("ingestFromStream: stream must be readable");
  }
  const limit =
  ...
  const tempName = `tmp${await require("./util").token(12)}`;
  const reservationKey = `ingest:${tempName}`;
  const reservation = await reserveUpload(reservationKey, limit);
  const tempStorage = new StorageLocation({
    name: tempName,
    volumeId: reservation.volumeId,
  });
  let size = 0;
  const guard = new Transform({
    transform(chunk, _encoding, callback) {
      ...
      size += data.length;
      if (limit && size > limit) {
        const error = new Error(`Plugin upload exceeds ${limit} bytes`);
        error.code = "PLUGIN_UPLOAD_TOO_LARGE";
        callback(error);
        return;
      }
      hasher.update(data);
  ...
  try {
    await tempStorage.mkdir();
    await pipeline(stream, guard, await tempStorage.openWriteAt(0));
  }
  catch (error) {
    await tempStorage.rm().catch(() => {});
    await releaseUpload(reservationKey, reservation.volumeId);
  ...
  if (!size) {
    await tempStorage.rm().catch(() => {});
    await releaseUpload(reservationKey, reservation.volumeId);
    throw new Error("ingestFromStream: stream was empty");
  }
  ...
  if (storage) {
    ...
    try {
      ...
      console.error(
        `ingestFromStream: metadata extraction failed for ${name}:`,
        error.message,
      );
      storage = new StorageLocation({
        name: tempName,
    ...
    STORAGE.set(hash, storage);
    generateAssets(storage).catch(error =>
      console.error(
        `ingestFromStream: asset generation failed for ${name}:`,
        error.message,
      ),
    );
  }
