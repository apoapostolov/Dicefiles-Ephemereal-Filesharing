...
const { decorateNewRoomMember } = require("./member-age");

function toClientFile(file) {
  return decorateNewRoomMember(file.toClientJSON());
}

function sanitizeFile(file) {
  file = toClientFile(file);
  delete file.ip;
  delete file.admin;
...
class FileLister {
  constructor(room) {
    ...
    this.onadded = new CoalescedUpdate(1000, (files) => {
      files.forEach((e) => {
        const j = JSON.stringify(toClientFile(e));
  ...
  async undirty() {
    ...
    this.privileged = this.files.map(toClientFile);
    this.regular = this.files.filter((f) => !f.hidden).map(sanitizeFile);
    this.dirty = false;
  }
  ...
  convert(files, role, ip) {
    ...
    return files.filter((f) => !f.hidden || f.ip === ip).map(sanitizeFile);
  }
