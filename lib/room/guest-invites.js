...
const GUEST_INVITE_AUDIT_EVENTS = Object.freeze([
  "created",
  "redeemed",
  "revoked",
]);
...
function countActiveGuestInvites(list, now) {
  return normalizeGuestInvites(list).filter(
    (invite) => inviteRedeemability(invite, now).ok,
  ).length;
}

function guestInviteCapacity(list, maxActive, now) {
  const max = clampInt(maxActive, 25, 1, 1000);
  const active = countActiveGuestInvites(list, now);
  return {
    ok: active < max,
    active,
    max,
...
function inviteTokenHint(token) {
...
function normalizeGuestInviteAudit(raw, limit) {
  if (!Array.isArray(raw)) {
    return [];
  }
  ...
  for (const item of raw) {
    ...
    if (!GUEST_INVITE_AUDIT_EVENTS.includes(event) || !Number.isFinite(at)) {
      continue;
    }
    out.push({
      event,
      at,
      tokenHint: String(item.tokenHint || "…").slice(0, 32),
      label: String(item.label || "").slice(0, 80),
      actor: String(item.actor || "").slice(0, 64),
      uses: clampInt(item.uses, 0, 0, MAX_MAX_USES),
      maxUses: clampInt(item.maxUses, 1, 1, MAX_MAX_USES),
    });
  }
  return out.slice(-max);
}

function appendGuestInviteAudit(raw, event, details, limit) {
  if (!GUEST_INVITE_AUDIT_EVENTS.includes(event)) {
    throw new Error("Invalid guest invite audit event");
  }
  const d = details || {};
  const record = {
    event,
    at: Number.isFinite(d.at) ? d.at : Date.now(),
    tokenHint: d.tokenHint || inviteTokenHint(d.token),
    label: d.label || "",
    actor: d.actor || "",
    uses: d.uses,
    maxUses: d.maxUses,
  ...
  return normalizeGuestInviteAudit(
    normalizeGuestInviteAudit(raw, limit).concat(record),
    limit,
  );
}
