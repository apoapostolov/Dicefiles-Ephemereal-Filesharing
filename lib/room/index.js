...
class Room extends EventEmitter {
  ...
  grantGuestPass(guestKey, invite) {
    ...
    const exp =
      invite && invite.expiresAt != null && Number.isFinite(Number(invite.expiresAt))
  ...
  createGuestInviteLink(opts) {
    ...
    const list = this.getGuestInvites().concat(inv);
  ...
  revokeGuestInviteLink(token) {
    const next = revokeGuestInvite(this.getGuestInvites(), token);
  ...
  redeemGuestInviteToken(token, guestKey) {
    ...
    try {
      ...
      webhooks.dispatch("guest_invite_redeemed", {
        ...
        remaining: Math.max(
          0,
          result.invite.maxUses - result.invite.uses,
        ),
      });
    } catch (_) {
      /* optional */
  ...
  async collectLinkedClientFiles(client) {
    ...
    for (const entry of entries) {
      ...
      const roomName =
        entry.name || src.config.get("roomname") || srcId;
