...
class Room extends EventEmitter {
  ...
  get exportedRoomConfig() {
    ...
    c.push(["linkedRooms", normalizeLinkedRooms(this.config.get("linkedRooms"))]);
  ...
  async getFilesFor(client) {
    ...
    const linked = await this.collectLinkedClientFiles(client);
    if (!linked.length) {
      return local;
    }
    // Prefer local keys when both appear (should be rare — keys are global).
    const localKeys = new Set(local.map((f) => f.key));
    return local.concat(linked.filter((f) => !localKeys.has(f.key)));
  }
  ...
  async collectLinkedClientFiles(client) {
    const sources = normalizeLinkedRooms(this.config.get("linkedRooms"));
    if (!sources.length) {
      return [];
    }
    ...
    for (const srcId of sources) {
      ...
      const roomName = src.config.get("roomname") || srcId;
  ...
  /**
   * Resolve linked room tokens (ids and/or names) and store room ids only.
   * @param {unknown} list
  ...
  async setLinkedRooms(list) {
    ...
    const { ids, unresolved } = resolveLinkedRoomTokens(
      list,
      catalog,
      this.roomid,
    );
    ...
    const filtered = ids.filter((id) => id !== this.roomid);
