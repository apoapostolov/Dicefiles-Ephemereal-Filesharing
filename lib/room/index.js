...
const PASSWORD_ACCESS = require("./password-access");
...
class Room extends EventEmitter {
    ...
    for (const roomid of roomids) {
      ...
      try {
        ...
        const room = {
          ...
          inviteonly: config.get("inviteonly") === true,
          passwordProtected: !!(
            config.get("passwordAccess") &&
            config.get("passwordAccess").enabled
          ),
        };
  ...
  static async preparePasswordAccess() {
    ...
    for (const summary of rooms) {
      ...
      try {
        const room = await Room.get(summary.roomid);
        if (room) {
          await PASSWORD_ACCESS.ensureCurrent(room);
          prepared++;
        }
      }
      catch (error) {
        console.error(
          `Failed preparing password rotation for ${summary.roomid}:`,
          error.message || error,
        );
      }
    }
  ...
  get exportedRoomConfig() {
    ...
    if (!this.config.has("passwordAccess")) {
      c.push([
        "passwordAccess",
        Object.assign(
          { enabled: false },
          CONFIG.get("roomPasswordAccess") || {},
        ),
      ]);
    }
  ...
  async getPasswordAccessAdminState(reveal = true) {
    await this[LOADING];
    return PASSWORD_ACCESS.adminState(this, reveal);
  }

  async configurePasswordAccess(arg) {
    await this[LOADING];
    const state = await PASSWORD_ACCESS.configure(this, arg);
    if (state.enabled) {
      this.config.set("allowCrossLinking", false);
      this.config.set("allowPrivateCrossLinking", false);
      this.config.set("allowFederation", false);
      this.config.set("allowPrivateFederation", false);
    }
    await redis.del("roomlist");
    return state;
  }

  async rotatePasswordAccess(arg) {
    await this[LOADING];
    return PASSWORD_ACCESS.rotateNow(
      this,
      arg && typeof arg === "object" ? arg.password : arg,
    );
  }

  async passwordAccessAllowed(user, roomToken, cookies) {
    await this[LOADING];
    return PASSWORD_ACCESS.allowed(this, user, roomToken, cookies);
  }

  async authenticatePasswordAccess(password, roomToken) {
    await this[LOADING];
    return PASSWORD_ACCESS.authenticate(this, password, roomToken);
  }
  ...
  async collectLinkedClientFiles(client) {
    ...
    for (const entry of entries) {
      ...
      // Password-protected rooms never leak through linked-room aggregation.
      if (
        src.config.get("passwordAccess") &&
        src.config.get("passwordAccess").enabled === true
      ) {
        continue;
      }
      if (
      ...
      try {
        ...
        if (src) {
          ...
          privateSource =
            ...
            !!(
              src.config.get("passwordAccess") &&
              src.config.get("passwordAccess").enabled
            );
  ...
  setAllowFederation(enabled) {
    if (
      enabled &&
      this.config.get("passwordAccess") &&
      this.config.get("passwordAccess").enabled
    ) {
      throw new Error(
        "Password-protected rooms cannot be federation sources",
      );
    }
  ...
  setAllowCrossLinking(enabled) {
    if (
      ...
      throw new Error(
        "Password-protected rooms cannot be cross-linking sources",
      );
    }
