...
class RequestFile {
  constructor(options) {
    ...
    if (!Number.isFinite(expires) || expires <= uploaded || expires > maxExpires) {
    ...
    if (!this.meta.requestImageDataUrl || typeof this.meta.requestImageDataUrl !== "string") {
      this.meta.requestImageDataUrl = "";
    }
    if (typeof this.status !== "string" || !(this.status === "open" || this.status === "fulfilled")) {
      this.status = "open";
    }
...
const REQUESTS = new DistributedMap("upload:requests", v => new RequestFile(v));
