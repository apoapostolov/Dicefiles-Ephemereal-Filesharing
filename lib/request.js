...
class RequestFile {
  constructor(options) {
    ...
    if (
      typeof this.fulfilledAt !== "number" ||
      !Number.isFinite(this.fulfilledAt) ||
      this.fulfilledAt < 0
    ) {
      this.fulfilledAt = 0;
    }
...
const EMITTER = new (class RequestEmitter extends EventEmitter {
  ...
  async setStatus(key, status, byNick) {
    ...
    data.fulfilledAt = status === "fulfilled" ? Date.now() : 0;
    const updated = new RequestFile(data);
    REQUESTS.set(key, updated);
    if (req.status !== "fulfilled" && status === "fulfilled") {
      OBS.trackRequestFulfilled(updated);
    }
    return updated;
  }
