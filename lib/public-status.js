...
function summarizeFederation(peers, enabled) {
  const rows = Array.isArray(peers) ? peers : [];
  const active = rows.filter(peer => peer && peer.status === "active");
  const unavailable = rows.filter(peer => peer && peer.status !== "active");
  const latencyMs = median(
    active.
      map(peer => finiteOrNull(peer.latencyMs)).
...
function buildSnapshot({
  ...
  federation = summarizeFederation([], false),
}) {
  ...
  const components = safeHealth.components.slice();
  if (federation.enabled) {
    components.push({
      id: "federation",
      label: "Trusted peers",
      status: federation.status,
      latencyMs: federation.latencyMs,
    });
  }
...
async function collect({ Room, UPLOAD, REQUEST }) {
  ...
  inFlight = (async () => {
    ...
    const federationEnabled =
      ...
      CONFIG.get("federation").enabled === true;
    const federationPromise = federationEnabled ?
      require("./federation/transport").listPeerStatuses() :
      Promise.resolve([]);
    const [
      roomsResult,
      usersResult,
    ...
    const snapshot = buildSnapshot({
      ...
      persisted,
      federation: summarizeFederation(
