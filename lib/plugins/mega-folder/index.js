 ...
 *   download() -> Promise<Buffer|Readable>
 *
...
/** In-process dedupe: roomId|folderUrl -> Set of "name:size" */
...
function entryDedupeKey(entry, name) {
  const size = entry && entry.size != null ? entry.size : "";
  return `${name}:${size}`;
}
