...
⚠️ **This is the original README. For the updated documentation, please see [README.md](../README.md).**

---

dicefiles - somewhat like volafile
===

dicefiles is like volafile, except open source and you can host it yourself.
...
dicefiles will load configuration data from all of these files, if they are present, in this order.
The last value for any configuration option wins (much like CSS rules).

1. `defaults.js` (the defaults of dicefiles, do not edit, but use as a reference of available options)
...
1. *My previews do not work!*
   ...
   Then it might be firejail that is broken, e.g. it refuses to run in another "sandbox" (which might be the VPS/linux container/docker your dicefiles is running in).
